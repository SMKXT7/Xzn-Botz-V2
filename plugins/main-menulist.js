...
rows: [
{title: `⚡ ${emot} SPEED TEST`, rowId: ".speed", description: "Menampilkan kecepatan respon BOT"},
{title: `👨‍💻 ${emot} OWNER BOT`, rowId: ".owner", description: "Menampilkan List owner BOT"},
{title: `🎟️ ${emot} SOURCE CODE`, rowId: ".sc", description: `Source Code ${namebot}`},
]
},{
title: `${htki} SUPPORT ${htka}`,
rows: [
{title: `🛍️ ${emot} SEWA`, rowId: ".sewa", description: "Menampilkan list harga sewa BOT"},
...
let psan = 'bagaimana kabarmu hari ini?'
...
┡────────────┈ ⳹
┠━ꕥ「 𝙸𝙽𝙵𝙾 𝚄𝚂𝙴𝚁 ☕︎ 」
┋❀ 𝐍𝐚𝐦𝐞: ${usrs.registered ? usrs.name : conn.getName(m.sender)}
...
┊ 「 𝙸𝙽𝙵𝙾 𝚂𝚃𝙰𝚃𝚄𝚂 𑁍 」
┊☁︎ 𝐔𝐩𝐭𝐢𝐦𝐞: ${mpt}
┊☁︎ 𝐓𝐢𝐦𝐞: ${moment.tz('Asia/Jakarta').format('HH')} H${moment.tz('Asia/Jakarta').format('mm')} M${moment.tz('Asia/Jakarta').format('ss')} S
┊☁︎ 𝐔𝐬𝐞𝐫𝐬: ${Object.keys(global.db.data.users).length}
┊☁︎ 𝐋𝐢𝐦𝐢𝐭: ${usrs.limit}
┊☁︎ 𝐋𝐞𝐯𝐞𝐥: ${usrs.level}
┊☁︎ 𝐑𝐨𝐥𝐞 ${usrs.role}${usrs.premiumTime > 1 ? `
┗––––––––––––––––––ꕥ
┊✈︎ 𝐄𝐱𝐩𝐢𝐫𝐞𝐝 𝐏𝐫𝐞𝐦𝐢𝐮𝐦:
...
const listMessage = {
text: tek,
footer: '⚠︎ *Note:* Jika menemukan bug, error atau kesulitan dalam penggunaan silahkan laporkan/tanyakan kepada Owner',
mentions: await conn.parseMention(tek),
title: `${htki} *LIST MENU* ${htka}`,
buttonText: `𝚃𝙾𝚄𝙲𝙷 𝙼𝙴 𝙾𝙽𝙸𝙲𝙷𝙰𝙽 𖧷`, 
sections
}
if (teks == '404') {
...
ptt: false, seconds: 0,contextInfo: {
 ...
 mediaUrl: sgc,
    mediaType: 2, 
    description: sgc,
    title: "ᴋᴀᴍᴜ ɴʏᴀᴛᴀ ᴛᴇᴛᴀᴘɪ ᴛɪᴅᴀᴋ ʙɪsᴀ ᴅɪᴍɪʟɪᴋɪ 𖠌",
    body: wm,
    thumbnail: await (await fetch('https://telegra.ph/file/ce6280ac071e15e38d305.jpg')).buffer(),
    sourceUrl: sgc
 	  /*   sourceUrl: sig,
           title: '◄⟬ ●━━━ ⧏ ⧎ ⧐ ━━━● ⟭►',  
            body: 'Now Playing...', 
